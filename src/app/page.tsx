import Link from "next/link";
import { BrandMark } from "@/components/ui/BrandMark";
import { ArrowRightIcon, PlayIcon } from "@/components/ui/icons";
import { getLesson, listLessons } from "@/lib/lessons";
import { SceneBackdrop } from "@/components/media/SceneBackdrop";

/**
 * THE PILOT HUB — the three videos for the §4.1 pilot review, in the order the
 * Agreement lists them (B §5): claims-01, siu-01, ew-01.
 *
 * These point at the NATIVE avatar cuts (Kling AI Avatar v2 Pro — movement and
 * lip-sync generated together), which is what the pilot review is now shown on.
 * The earlier dubbed cuts are NOT deleted; they remain reachable at their own
 * slugs (claims-investigation-application-1, siu-01-av1, ew-01-av1) — this hub
 * simply features the native cuts. To revert, restore the three slugs below.
 *
 * Named explicitly rather than taken from `listLessons()` because the registry also
 * carries `water-damage-claim`, the TEMPLATE PROOF (not pilot content), plus the
 * dubbed cuts and any preview variants, none of which must be presented as a pilot.
 * Adding video #4 of the catalog means adding its slug here.
 *
 * Deliberately NOT labelled with presenter names. On-screen presenter identification
 * is locked to the fixed lower-third "Course Presenter" (Agreement §1.4 / Spec B §6),
 * and the per-lesson instructor credential was removed on purpose. Each card carries
 * that lesson's own presenter still, so the three are told apart by sight without
 * introducing a presenter identity the locked template does not carry.
 */
const PILOT_SLUGS = [
  "claims-01-av1-native",
  "siu-01-av1-native",
  "ew-01-av1-native",
] as const;

export default function Home() {
  const bySlug = new Map(listLessons().map((l) => [l.slug, l]));
  const pilots = PILOT_SLUGS.flatMap((slug) => {
    const summary = bySlug.get(slug);
    const lesson = getLesson(slug);
    if (!summary || !lesson) return [];
    const start = lesson.scenes[lesson.startSceneId];
    return [{ ...summary, poster: start?.media?.posterUrl }];
  });

  return (
    <div className="relative min-h-[100dvh] w-full overflow-hidden bg-navy-950">
      {/* atmospheric backdrop */}
      <div className="absolute inset-0 opacity-70">
        <SceneBackdrop scene="claim-desk" animate={false} />
      </div>
      <div className="absolute inset-0 bg-gradient-to-b from-navy-950/40 via-navy-950/70 to-navy-950" />

      <div className="relative mx-auto flex min-h-[100dvh] w-full max-w-6xl flex-col px-5 py-6 sm:px-8">
        <header className="flex items-center justify-between">
          <BrandMark size="md" />
          <nav className="hidden items-center gap-6 font-sans text-sm text-ink-300 sm:flex">
            <span className="cursor-default">Curriculum</span>
            <span className="cursor-default">Faculty</span>
            <span className="cursor-default">Accreditation</span>
            <Link
              href="#pilots"
              className="whitespace-nowrap rounded-full border border-gold-500/50 px-4 py-1.5 text-gold-200 transition-colors hover:bg-gold-500/10"
            >
              Enter a case
            </Link>
          </nav>
        </header>

        <main className="flex flex-1 flex-col justify-center py-14">
          <div className="max-w-3xl">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/5 px-3 py-1 font-sans text-[11px] font-medium uppercase tracking-[0.2em] text-gold-200">
              <span className="h-1.5 w-1.5 rounded-full bg-gold-400" />
              Interactive Case Studies
            </div>
            <h1 className="font-[family-name:var(--font-display)] text-4xl font-medium leading-[1.05] tracking-tight text-ink-100 sm:text-6xl">
              Judgment, trained the way{" "}
              <span className="text-gold-400">the work actually happens.</span>
            </h1>
            <p className="mt-5 max-w-xl font-sans text-lg leading-relaxed text-ink-200">
              Eximious Academy turns real claims and investigation scenarios into
              branching, decision-driven video lessons. Work the evidence, make
              the call, and see exactly why it was right — or wasn&apos;t.
            </p>

            <div className="mt-8 flex flex-wrap items-center gap-4">
              <Link
                href="#pilots"
                className="group inline-flex items-center gap-2 rounded-full bg-gold-500 px-7 py-3.5 font-sans text-[15px] font-semibold text-navy-950 shadow-lg shadow-gold-900/40 transition-colors hover:bg-gold-400"
              >
                Choose a case study
                <ArrowRightIcon className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
              </Link>
              <span className="font-sans text-sm text-ink-400">
                {pilots.length} case studies · 3 decision points each · 12 feedback branches
              </span>
            </div>
          </div>

          {/* the three pilot case studies */}
          <div id="pilots" className="mt-14 scroll-mt-8">
            <div className="mb-3 font-sans text-[11px] font-semibold uppercase tracking-[0.24em] text-ink-400">
              Case studies
            </div>
            <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {pilots.map((p) => (
                <li key={p.slug}>
                  <Link
                    href={`/lesson/${p.slug}`}
                    className="group flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-navy-900/50 backdrop-blur-sm transition-all hover:border-gold-500/40 hover:bg-navy-900/70"
                  >
                    <div className="relative aspect-video w-full shrink-0 overflow-hidden border-b border-white/10">
                      {p.poster && (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img
                          src={p.poster}
                          alt=""
                          className="absolute inset-0 h-full w-full object-cover object-top"
                        />
                      )}
                      <div className="absolute inset-0 bg-navy-950/25" />
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="flex h-11 w-11 items-center justify-center rounded-full bg-navy-950/55 ring-1 ring-white/25 backdrop-blur-sm transition-transform group-hover:scale-105">
                          <PlayIcon className="ml-0.5 h-4 w-4 text-ink-100" />
                        </span>
                      </div>
                      <span className="absolute left-2 top-2 rounded bg-navy-950/70 px-1.5 py-0.5 font-sans text-[9px] font-semibold uppercase tracking-[0.15em] text-gold-300 ring-1 ring-white/10">
                        Case study
                      </span>
                      {p.estimatedMinutes && (
                        <span className="absolute bottom-2 right-2 rounded bg-navy-950/70 px-1.5 py-0.5 font-sans text-[9px] font-medium text-ink-200 ring-1 ring-white/10">
                          {p.estimatedMinutes} min
                        </span>
                      )}
                    </div>
                    <div className="flex min-w-0 flex-1 flex-col p-4">
                      <div className="font-sans text-[11px] uppercase tracking-[0.18em] text-gold-300">
                        {p.courseTitle}
                      </div>
                      <div className="mt-1 font-[family-name:var(--font-display)] text-lg leading-snug text-ink-100">
                        {p.title}
                      </div>
                      {p.subtitle && (
                        <div className="mt-1 font-sans text-sm leading-snug text-ink-300">
                          {p.subtitle}
                        </div>
                      )}
                      <div className="mt-auto flex items-center justify-between pt-3 font-sans text-[11px] text-ink-400">
                        <span>
                          {p.caseId ? `Case ${p.caseId}` : `${p.decisionCount} decisions`}
                        </span>
                        <ArrowRightIcon className="h-4 w-4 shrink-0 transition-all group-hover:translate-x-0.5 group-hover:text-gold-300" />
                      </div>
                    </div>
                  </Link>
                </li>
              ))}
            </ul>
          </div>
        </main>

        <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-white/8 pt-4 font-sans text-[11px] text-ink-400">
          <span>© Eximious Academy · Professional Claims Education</span>
          <span className="hidden sm:block">Investigation · Coverage · Litigation Readiness</span>
        </footer>
      </div>
    </div>
  );
}
